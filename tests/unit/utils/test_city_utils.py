...
GITHUB_CSV_URL = "https://raw.githubusercontent.com/AlvaroM99/spanish_capital_cities/main/raw_cities.csv"
...
def test_caching_mechanism():
    ...
    sample_df = pd.DataFrame([
        {'city_code': '28079', 'city_name': 'Madrid',
         'latitud': 40.4168, 'longitud': -3.7038,
         'country_code': 'ES', 'is_coastal': 0}
    ])
...
def test_get_capitals_dataframe_format(sample_github_csv):
    ...
    assert pd.api.types.is_integer_dtype(capitals_df['city_code']) or pd.api.types.is_string_dtype(capitals_df['city_code'])
