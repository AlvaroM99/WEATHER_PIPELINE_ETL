"""
Transform Open-Meteo Air Quality Data
...
class OpenMeteoAirQualityTransformer(BaseTransformer):
    """
    Transformer for Open-Meteo Air Quality data.
    """

    def transform(self, **context):
        """Transform Open-Meteo air quality from JSON to Parquet"""
        self.log_start("Transforming Open-Meteo air quality (Bronze → Silver)")
        
        # Intento de recuperar XCom
        bronze_objects = None
        execution_date = None
        
        for task_id in ['extract_openmeteo_air_quality', 'extract_air_quality_data']:
            try:
                bronze_objects = context['task_instance'].xcom_pull(key='openmeteo_air_quality_objects', task_ids=task_id)
        ...
        if not bronze_objects:
            ...
            prefix = f"air_quality/{execution_date}/"
            try:
        ...
        if not bronze_objects:
            self.logger.warning("No air quality data to transform")
            return 0
        ...
        for bronze_obj in bronze_objects:
            try:
                ...
                self.log_error(f"Error transforming air quality file {bronze_obj.get('object_path')}", e)
                continue
        ...
        silver_path = f"air_quality/{execution_date}/air_quality_{timestamp}.parquet"
        file_size = minio_client.upload_parquet(SILVER_OPENMETEO_BUCKET, silver_path, combined_df)
        
        self.log_end(f"Wrote {len(combined_df)} air quality records to silver ({file_size} bytes)")
...
def transform_openmeteo_air_quality(**context):
    """
    Wrapper function for Airflow PythonOperator
    """
    transformer = OpenMeteoAirQualityTransformer()
