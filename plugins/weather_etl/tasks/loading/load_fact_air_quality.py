"""
Load Fact Table: Air Quality
Reads from MinIO silver-openmeteo (Parquet) and loads into dwh.fct_air_quality
"""
import logging
import pandas as pd
...
class AirQualityLoader(BaseLoader):
    """
    Loader for Air Quality fact table.
    """
    ...
    def load(self, **context):
        """
        Load air quality data from MinIO silver layer to fact table
        """
        self.log_start("Loading fct_air_quality (Silver → Gold)")
        ...
        try:
            ...
            silver_path = f"air_quality/{execution_date}/"
            ...
            for _, row in df.iterrows():
                ...
                records.append((
                    ...
                    get_val('pm10'),
                    get_val('pm2_5'),
                    get_val('carbon_monoxide'),
                    get_val('nitrogen_dioxide'),
                    get_val('sulphur_dioxide'),
                    get_val('ozone'),
                    get_val('aerosol_optical_depth'),
                    get_val('dust')
                ))
            ...
            self.log_end(f"Inserted {inserted_count} records into fct_air_quality")
            ...
            conn.rollback()
            self.log_error("Error loading air quality", e)
            raise
        finally:
            conn.close()


def load_fact_air_quality(**context):
    """
