...
from src.data_quality.validators import ValidationResult, DataQualityValidator
...
DAILY_FORECAST_CONFIG = {
    'required_columns': [
        'city_name', 'time', 'temperature_2m_max', 'temperature_2m_min'
    ],
    'numeric_ranges': {
...
def _run_validation(
    ...
    # Log summary
    if result.success:
        logger.info(f"[{suite_name}] Validation PASSED: {result.successful_expectations}/{result.total_expectations} checks passed")
    else:
        logger.warning(f"[{suite_name}] Validation FAILED: {result.failed_expectations}/{result.total_expectations} checks failed")
